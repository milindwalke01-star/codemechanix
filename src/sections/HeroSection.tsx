...
const HeroSection = ({ onOpenContact }: HeroSectionProps) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.hero-subheadline', { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 }, '-=0.4');
      tl.fromTo('.hero-cta', { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 }, '-=0.3');
      tl.fromTo('.hero-card', { y: 40, opacity: 0, scale: 0.95 }, { y: 0, opacity: 1, scale: 1, duration: 0.6 }, '-=0.3');
      tl.fromTo('.hero-image', { scale: 1.1, opacity: 0 }, { scale: 1, opacity: 1, duration: 1 }, 0);
    }, sectionRef);
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen bg-[#0B0F17] flex items-center pt-16 lg:pt-0">
      <div
        className="aurora-blob bg-[#4F46E5]"
        style={{ width: '600px', height: '600px', right: '-200px', top: '10%' }}
      />
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center max-w-7xl mx-auto">
          <div ref={contentRef} className="order-2 lg:order-1">
            <h1 className="hero-headline text-3xl sm:text-4xl lg:text-5xl xl:text-6xl leading-tight text-[#F5F7FF] mb-4 lg:mb-6">
              Small business ops, <span className="text-[#4F46E5]">automated.</span>
            </h1>

            <p className="hero-subheadline text-base lg:text-lg text-[#A9B3C7] leading-relaxed mb-6 lg:mb-8 max-w-lg">
              We build custom tools, workflows, and micro-apps that save hours every week—without hiring more staff.
            </p>
